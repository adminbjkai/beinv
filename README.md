...
| dir | what | run |
|---|---|---|
| [`tv/`](tv/) | **Apple TV app** (SwiftUI + AVKit). Catalog from beIN; HD + La Liga play from `beinv.bjk.ai`. v2.8 simulator-tested; v2.7.1 remains the last physical Apple TV install. | [NATIVE_RESUME.md](docs/NATIVE_RESUME.md) |
| [`android/`](android/) | **Android app** (Kotlin, Compose, Media3 ExoPlayer). Same split. v2.8 debug APK built and walked on Pixel_9. | [android/README.md](android/README.md) · [NATIVE_RESUME.md](docs/NATIVE_RESUME.md) |
| [`server/`](server/) + [`web/`](web/) | **Web app**: Rust (axum) API/video proxy + responsive React SPA. v2.8 browser-tested locally. | see below |

---

## Apple TV app (`tv/`)

v2.8 is built, installed, UI-tested, and visually checked on the Apple TV 4K simulator. v2.7.1 is the last build installed on the Living Room Apple TV 4K. Build notes and the device checklist: [docs/NATIVE_RESUME.md](docs/NATIVE_RESUME.md).

- tvOS 17+, SwiftUI, AVKit `AVPlayerViewController` over an `AVQueuePlayer` (native transport bar, Siri Remote, no external packages).
- Files: `HighlightsApp.swift` (entry), `Models.swift` (lenient Codable models, leagues, `Mode`, `MatchEvent.eventTeamSide` → `Side`, `Match.goalRows` running-score computation, `Clip` playlist item with week / running score / team logo, `orderedGoalPlaylist` — the single §2b ordering), `API.swift` (actor: seasons + week GETs, season-wide TaskGroup fetch ~6 weeks at a time, in-memory caches per week/season), `BrowseView.swift` (§0 layout, compact HD control, focus-safe selection markers, contained week rail, polished cards/pickers and structured loading/empty/error states), `PlayerView.swift` (queue player plus a taller, accessible week-grouped native Clips info panel and the press-and-hold clip picker), `Theme.swift` (charcoal `#0B0F0E`, emerald `#19C37D`).
...
- UI tests (`HighlightsUITests/HighlightsUITests.swift`, target `HighlightsUITests`) drive the app with `XCUIRemote`: season/mode navigation, Goals/Play all → AVKit → Clips jump → Menu return, and a dedicated HD focus/action regression. Controls retain stable `accessibilityIdentifier`s (`league.<id>`, `season.button`, `week.all`, `hd.toggle`, `mode.<highlights|goals|team>`, `team.button`, `sub.matches/goals`, `only.toggle`, `goals.playall`, `player.view`, `player.clips`, `clip.<i>`, `picker.row.<i>`, `goal.<id>`, `empty`). Run: `cd tv && xcodegen generate && xcodebuild test -project Highlights.xcodeproj -scheme Highlights -destination 'platform=tvOS Simulator,name=Apple TV 4K (3rd generation)' -derivedDataPath build CODE_SIGNING_ALLOWED=NO`.
...
v2.8 debug APK assembled, installed, and walked on Pixel_9. Checklist: [docs/NATIVE_RESUME.md](docs/NATIVE_RESUME.md).
...
- Browse: full-name scrollable league chips on phones (segmented on wide screens), season dropdown, HD switch, auto-scrolling week rail, and an adaptive card grid. Cards have stable logo fallbacks and structured loading/empty/error states. Tap a card → player with an accessible clip list, explicit playback-error Retry, fullscreen drawer, and PiP.
...
The `l/s/r` query lets a cold server re-fetch the week to find the video source (`r` is the match's own `round`, so it also works for matches found via the season route). It is optional and only consulted on a cache miss, so a warm server answers a bare `/video/{kind}/{id}`. The SPA keeps state in the URL and mirrors it to `localStorage` (`beinv.v2`); URL wins over storage. Params: `l` league · `s` season id · `r` week (round) or `all` · `hd=1`/`hd=0` Super Lig / Premier League remux (default on; last choice is remembered) · `mode` `goals` | `team` (omitted = Highlights) · `t` team name (By team) · `g=1` Goals sub-switch in By team · `og=0/1` "Only <Team> goals" toggle (default `1`, only written when `g=1`) · `m` open match id · `play=all` open "Play all" for the visible goals on load · `clips=1` open the in-player clip selector on load (the last two are read once and not written back). The autoplay-next toggle is stored only in `localStorage`.

Views (mode toggle): **Highlights** (match cards for All weeks, grouped by week, or one round) · **Goals** (goal clips grouped by match, "Play all" runs them as one playlist) · **By team** (team picker derived from the season's matches; that team's matches newest-first labelled by week, with a Matches / Goals sub-toggle and an "Only <Team> goals" toggle). Layout order follows [FEATURES.md §0](docs/FEATURES.md): League → Season → Mode → HD (Super Lig / Premier League) → Week rail (hidden in By team) → content; picking a season resets to **All weeks**. `[` / `]` steps the week rail.
...
beIN serves one progressive H.264 mp4 per highlight/clip (the only rendition → highest quality). Native decoders + byte-range streaming is the smoothest path; no HLS/ABR. La Liga 2025/2026 and 2026/2027 official highlights are remuxed to the same MP4 contract (best H.264 + AAC, typically 1080p50, `+faststart`) so the in-app player never loads an external video site. Trendyol Süper Lig and Premier League have an **HD** toggle (on by default) that remuxes the official beIN SPORTS Türkiye / NBC Sports YouTube özet the same way; goal clips stay on the beIN mp4.

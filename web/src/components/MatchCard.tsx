...
export default function MatchCard({ m, onOpen, i, label }: { m: Match; onOpen: () => void; i: number; label?: string }) {
  ...
  return (
    <button onClick={onOpen} style={{ animationDelay: `${Math.min(i, 12) * 40}ms` }}
      className="fade-up glass control-surface group relative overflow-hidden rounded-2xl text-left transition duration-300 hover:-translate-y-1 hover:border-accent/40 hover:shadow-[0_20px_60px_-20px_rgba(25,195,125,.4)] focus-visible:border-accent">
      <div className="relative aspect-video overflow-hidden bg-black/40">
        {m.thumb && <img src={m.thumb} alt="" loading="lazy" decoding="async" className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />}
        ...
        <span className="absolute left-3 top-3 flex max-w-[calc(100%-5rem)] gap-1.5">
          {label && <span className="rounded-md bg-accent/90 px-2 py-0.5 text-[11px] font-semibold text-black backdrop-blur">{label}</span>}
          <time dateTime={m.date} className="truncate rounded-md bg-black/65 px-2 py-0.5 text-[11px] font-medium text-white/80 backdrop-blur">{dateFmt.format(new Date(m.date))}</time>
        </span>
        {goals > 0 && (
      ...
      <div className="grid grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-center gap-3 p-4">
        <Side t={m.home} />
...
function Side({ t, right }: { t: Match['home']; right?: boolean }) {
  return (
    <div className={`flex min-w-0 flex-1 items-center gap-2 ${right ? 'flex-row-reverse text-right' : ''}`}>
      ...
      <span className="line-clamp-2 text-sm font-semibold leading-tight">{t.name}</span>
    </div>
  )
}
